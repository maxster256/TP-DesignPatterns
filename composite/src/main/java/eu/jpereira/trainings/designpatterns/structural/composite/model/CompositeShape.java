...
public abstract class CompositeShape extends Shape {
	...
	/**
	 * @param desiredShapeType
	 * @return
	 */
	public List<Shape> getShapesByType(ShapeType desiredShapeType) {
		List<Shape> shapesOfDesiredType = new ArrayList<Shape>();

		for (Shape shape: shapes)
			if (shape.getType().equals(desiredShapeType))
				shapesOfDesiredType.add(shape);

		return shapesOfDesiredType;
	}
	...
	public List<Shape> getLeafShapes() {
		...
		for (Shape shape: shapes)
			if (LeafShape.class.isAssignableFrom(shape.getClass()))
